      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-md border border-indigo-50 p-4 flex flex-col gap-6">
        ...
        <div className="border p-4 rounded-lg bg-gray-50">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            ...
            {/* Toggle زر تشغيل احترافي */}
            <label className="flex items-center cursor-pointer select-none">
              <span className="mr-3 text-base font-medium text-gray-700">Enable All Settings</span>
              <div
                className={`w-14 h-7 flex items-center rounded-full p-1 transition-colors duration-300 ${settings.enabled ? "bg-indigo-600" : "bg-gray-300"}`}
                onClick={() => setSettings(prev => ({ ...prev, enabled: !prev.enabled }))}
              >
                <div className={`bg-white w-6 h-6 rounded-full shadow-md transform duration-300 ease-in-out ${settings.enabled ? "translate-x-7" : "translate-x-0"}`} />
              </div>
            </label>
